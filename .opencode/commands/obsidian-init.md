...
4. Generate a complete `_CLAUDE.md` using the template in `~/.opencode/skills/obsidian-second-brain/references/claude-md-template.md`, filled with real values from the vault
...
7. Create `Bases/` at the vault root if it does not exist. Stamp the four premade base files from `~/.opencode/skills/obsidian-second-brain/references/bases/`:
...
If a monolithic `log.md` already exists with `## YYYY-MM-DD` sections: run `python ~/.opencode/skills/obsidian-second-brain/scripts/migrate_log.py --vault <vault-path>` to split it into `Logs/YYYY-MM-DD.md` files. Do not overwrite manually.
